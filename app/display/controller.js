...
app.controller('displayCtrl', ['$scope', '$http', function($scope, $http) {
	...
	$scope.showAddSlotDialog = function(slot_no) {
		...
		$http({
			...
			$scope.slot_media = response.data.records;
		});
		// show_slot_modal();
	}
	...
	$scope.showSwapSlotDialog = function(slot_no, media_id) {
		...
		$http({
			...
			$scope.slot_media = response.data.records; // media that came from the server
		});
	}
	...
	$scope.readOne = function(id){
		...
		/* $http({
			method: 'POST',
			data: { 'id' : id },
			url: 'api/display/read_one.php'
		}).then(function successCallback(response) {
	 
			// put the values in form
			$scope.id = response.data[0]["display_id"];
			$scope.name = response.data[0]["display_name"];
			$scope.slot_count = response.data[0]["slot_count"];
		...
		$http({
			method: 'GET',
			// data: { 'id' : id },
			url: 'api/display/read_one.php?id='+id
		}).then(function successCallback(response) {
			// put the values in form
