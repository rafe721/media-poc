...
function $id(id) {
	...
	function FileSelectHandler(e) {
		...
		// files[0].type.indexOf("Video/") < 0 || files[0].type.indexOf("video/") < 0
		if (files[0].type.indexOf("video/") < 0 && files[0].type.indexOf("image/") < 0) {
			notify('Please add/drop an image or a video file.');
		} else {
	...
	function file_init() {
		...
		var choose_file = $id('choose_file');
		files = null;
		setOutput('<p>No Files Uploaded</p>');
		
		// file select
		fileselect.addEventListener("change", FileSelectHandler, false);
		choose_file.addEventListener("click", file_choose, false);
		
		// is XHR2 available?
		var xhr = new XMLHttpRequest();
		if (xhr.upload) {
			$drag.css('display', 'inline-block'); // make Drag visible
			
			$drag.on('dragover dragleave', FileDragHover);
			$drag.on('drop', FileSelectHandler);
			// $upload_button.on('click', uploadFile);
			$('#file_upload').off().on('click', uploadFile);
